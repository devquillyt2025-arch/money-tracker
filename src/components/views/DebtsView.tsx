...
export default function DebtsView({
  ...
  const getNextDueDate = (debt: Debt) => {
    ...
    const numPaid = Math.max(paymentHistory.filter(p => p.debtId === debt.id).length, Math.floor((debt.totalAmount - debt.remainingAmount) / debt.emi));
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        {sortedDebts.map(debt => {
          ...
          return (
          <div key={debt.id} className={`bg-white dark:bg-gray-900 border ${isClosed ? 'border-gray-100 dark:border-gray-800/50 opacity-60' : isOverdue ? 'border-red-300 dark:border-red-800 shadow-sm' : 'border-gray-200 dark:border-gray-800 hover:border-blue-200 dark:hover:border-blue-800'} p-5 rounded-2xl shadow-sm transition-colors`}>
            <div className="flex flex-col md:flex-row justify-between gap-4 mb-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <h3 className={`font-sans font-semibold text-lg ${isClosed ? 'text-gray-500 dark:text-gray-400' : isOverdue ? 'text-red-700 dark:text-red-400' : 'text-gray-900 dark:text-gray-50'}`}>{debt.name}</h3>
                  {isOverdue && !isClosed && (
                    <span className="px-2 py-0.5 rounded-md border border-red-200 dark:border-red-800 text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 font-sans text-[10px] font-semibold">
                      OVERDUE
                    </span>
                  )}
                  <span className="px-2 py-0.5 rounded-md border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 font-sans text-[10px] uppercase tracking-wider text-gray-500 dark:text-gray-400 font-medium">
                    {debt.type.replace('_', ' ')}
                  </span>
                  {isClosed && (
                    <span className="px-2 py-0.5 rounded-md border border-green-200 dark:border-green-800 text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-900/20 font-sans text-[10px] font-semibold">
                      CLOSED
                    </span>
                  )}
                  {debt.interestRate > 0 && !isClosed && (
                    <span className={`px-2 py-0.5 rounded-md border font-sans text-[10px] font-semibold ${
                ...
                <div className="flex flex-col gap-0.5">
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    Original Amount: ₹{debt.totalAmount.toLocaleString('en-IN')}
                  </p>
                  {debt.emi !== undefined && debt.emi > 0 && !isClosed && (
                    <div className="flex flex-col gap-0.5">
                      <p className="text-sm font-medium text-gray-600 dark:text-gray-300">
                        Monthly EMI: ₹{debt.emi.toLocaleString('en-IN')}
                        {nextDue && (
                          <span className={`ml-2 text-xs font-semibold ${isOverdue ? 'text-red-600 dark:text-red-400' : 'text-gray-500 dark:text-gray-400'}`}>
                            Due {nextDue.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                          </span>
                        )}
                      </p>
                      <p className="text-xs text-blue-600 dark:text-blue-400 font-medium">
                        {Math.ceil(debt.remainingAmount / debt.emi)} EMIs remaining
                        {payoffDate && <span className="ml-1 font-normal opacity-80">(Paid off by {payoffDate})</span>}
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="flex flex-wrap items-center gap-4 md:justify-end">
                <div className="text-right">
                  <p className="font-sans text-xs font-medium text-gray-500 dark:text-gray-400 mb-0.5">Remaining</p>
                  <p className={`font-sans text-lg font-bold ${isClosed ? 'text-gray-400' : 'text-gray-900 dark:text-gray-50'}`}>₹{debt.remainingAmount.toLocaleString('en-IN')}</p>
                </div>
                {debt.emi !== undefined && debt.emi > 0 && !isClosed && (
                  <button
                    onClick={() => handleMarkEmiPaid(debt)}
                    className="px-3 py-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-800 rounded-xl text-xs font-semibold hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors"
                  >
                    Mark EMI Paid
                  </button>
                )}
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="p-2.5 bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-800 rounded-xl hover:border-blue-600 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 dark:bg-blue-900/20 transition-colors"
                  ...
                  <button
                    ...
                    className="p-2.5 text-gray-400 dark:text-gray-500 hover:text-blue-600 dark:text-blue-400 hover:bg-blue-50 dark:bg-blue-900/20 rounded-xl transition-colors"
                    title="Edit"
                  >
                    <Edit2 size={18} />
                  </button>
                  <button
                    ...
                    className="p-2.5 text-gray-400 dark:text-gray-500 hover:text-red-600 dark:text-red-400 hover:bg-red-50 dark:bg-red-900/20 rounded-xl transition-colors"
                    title="Delete"
                  >
                    <Trash2 size={18} />
                  </button>
        ...
        {debts.length === 0 && (
          <div className="text-center py-12 border-2 border-dashed border-gray-200 dark:border-gray-800 rounded-2xl bg-gray-50 dark:bg-gray-800/50">
